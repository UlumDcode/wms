...
const TarikDepositModal = ({
  ...
  const handleSubmit = async (e) => {
    ...
    const maxSaldo = parseFloat(
      selectedReseller.saldo_deposit ?? selectedReseller.saldo_titipan ?? 0,
    );

    if (cleanNominal <= 0) {
      return window.showToast(
        "Nominal penarikan tidak boleh kosong atau 0!",
        "warning",
      );
    }
    if (cleanNominal > maxSaldo) {
      return window.showToast("Saldo tidak mencukupi!", "warning");
    }
    if (!rekeningId) {
      return window.showToast("Pilih Rekening Sumber Pencairan!", "warning");
    }

    setLoading(true);
    try {
      const res = await axiosInstance.post(
        "finance/deposit_reseller.php?action=tarik_saldo",
        {
      ...
      if (data.status === "success") {
        window.showToast(
          `Tarik Saldo Rp ${cleanNominal.toLocaleString("id-ID")} Berhasil!`,
          "success",
        );
        onSuccess();
        onClose();
      ...
      <div className="bg-white w-full max-w-3xl rounded-xl shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
        <div className="p-4 border-b border-slate-100 bg-slate-50 text-center">
          <h3 className="font-black italic uppercase text-lg text-slate-800 tracking-widest">
            Tarik <span className="text-rose-600">Saldo</span>
          </h3>
          <p className="text-xs font-bold text-rose-600 mt-1 uppercase">
        ...
        >
          <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              className="w-full bg-slate-50 border border-slate-200 py-2 px-3 rounded-lg font-black text-sm text-rose-600 outline-none focus:border-rose-500 transition-all shadow-inner"
            />
          </div>

          <div className="flex flex-col gap-1.5">
            <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">
              Sumber Dana (Uang Keluar Dari)
            </label>
            <select
              value={rekeningId}
              onChange={(e) => setRekeningId(e.target.value)}
              className="w-full bg-white border border-slate-200 py-2 px-3 rounded-lg font-bold text-sm outline-none focus:border-rose-500 transition-all shadow-sm"
            >
          ...
          <div className="col-span-full flex justify-end gap-3 mt-2 md:mt-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-4 py-2 bg-rose-600 text-white rounded-lg font-black text-[10px] uppercase tracking-widest shadow-lg hover:bg-rose-700 active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
            >
...
export default TarikDepositModal;
