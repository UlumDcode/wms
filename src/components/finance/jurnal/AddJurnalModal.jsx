...
const AddJurnalModal = ({ isOpen, onClose, coaList = [], onSuccess }) => {
  if (!isOpen) return null;

  const [formMaster, setFormMaster] = useState({
    tanggal: new Date().toISOString().split("T")[0],
    no_referensi: "JUR-" + Date.now(),
    deskripsi: "",
  });

  const [formDetails, setFormDetails] = useState([
    { id: Date.now() + 1, coa_id: "", debit: "", kredit: "", keterangan: "" },
    { id: Date.now() + 2, coa_id: "", debit: "", kredit: "", keterangan: "" },
  ]);

  const [loading, setLoading] = useState(false);

  // LOGIKA KALKULASI
  const totalDebit = formDetails.reduce(
    (sum, d) => sum + (parseInt(d.debit) || 0),
    0,
  );
  const totalKredit = formDetails.reduce(
    (sum, d) => sum + (parseInt(d.kredit) || 0),
    0,
  );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axiosInstance.post(
        "finance/jurnal_umum.php?action=create",
        payload
      );

      const data = res.data;
      if (data.status === "success") {
        window.showToast("Jurnal berhasil disimpan!", "success");
        onSuccess();
        onClose();
      } else {
        window.showToast(data.message || "Gagal menyimpan jurnal.", "error");
      }
    } catch (error) {
      console.error(error);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormMaster({ ...formMaster, tanggal: e.target.value })
                  }
              ...
              <div className="md:col-span-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormMaster({ ...formMaster, deskripsi: e.target.value })
                  }
                  className="w-full bg-slate-50 border border-slate-200 py-2 px-3 rounded-lg font-bold text-sm focus:border-blue-500 shadow-sm outline-none transition-colors"
                        ...
                        <td className="py-2 px-2">
                          <div className="relative">
                            ...
                            <input
                              ...
                              className="w-full bg-slate-50 border border-slate-200 py-2 pl-8 pr-3 rounded-lg font-black text-sm focus:border-blue-500 shadow-sm outline-none text-emerald-600"
                              placeholder="0"
                            />
                          </div>
                        </td>
                        <td className="py-2 px-2">
                          <div className="relative">
                            ...
                            <input
                              ...
                              className="w-full bg-slate-50 border border-slate-200 py-2 pl-8 pr-3 rounded-lg font-black text-sm focus:border-blue-500 shadow-sm outline-none text-rose-600"
                              placeholder="0"
                            />
                          </div>
                        </td>
        ...
        <div className="p-4 md:p-6 bg-slate-50 border-t border-slate-100 shrink-0 flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-6 w-full md:w-auto text-center md:text-left">
            <div>
              ...
              <span
                className={`text-lg font-black ${!isBalance ? "text-rose-600" : "text-emerald-600"}`}
              >
            ...
            <div
              className={`hidden md:block ml-4 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest border ${isBalance ? "bg-emerald-50 border-emerald-200 text-emerald-600" : "bg-rose-50 border-rose-200 text-rose-600 animate-pulse"}`}
            >
          ...
          <div className="flex gap-3 w-full md:w-auto">
            ...
            <button
              ...
              className={`w-full md:w-auto px-8 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all shadow-md flex justify-center items-center gap-2 ${!isBalance || loading ? "bg-slate-300 text-slate-500 cursor-not-allowed opacity-50" : "bg-blue-600 text-white hover:bg-slate-900 active:scale-95"}`}
            >
